...
class RedisCache:
    ...
    async def put_row(self, key: str, payload: str) -> None:
        ...
        logging.info("Save {0} to cache. Type payload".format(type(payload)))
...
def cache(cached_obj: type[BaseApiModel]):
    def wrapp(func):
...
def cache_list(cached_obj: type[BaseApiModel]):
    def wrapp(func):
